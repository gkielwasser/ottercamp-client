...
angular.module('$strap.directives').directive('bsDatepicker', [
  ...
  '$strapConfig',
  '$rootScope',
  function ($timeout, $strapConfig,$rootScope) {
    'use strict';
        ...
        if (isAppleTouch) {
          ...
          if (controller) {
            element.on('changeDate', function (ev) {
              ...
              scope.$apply(function () {
              	//Ajout Pour récupérer le timeStamp
              	$rootScope.$broadcast('changeDate', ev.date.getTime()/1000);
    ...
    var ModalFactory = function ModalFactory(options) {
      function Modal(options) {
        ...
        return $q.when($templateCache.get(templateUrl) || $http.get(templateUrl, { cache: true }).then(function (res) {
          ...
          scope.$on('$destroy', function () {
          //Ajout gab -> permet de bien enelever le backdrop quand on détruit la modal
          	scope.hide();
            $modal.remove();
          });
          if (options.show) {
          ...
          updater: function (value) {
            if (controller) {
              scope.$apply(function () { 
                ...
                /* Fix tagsPicker*/
                if(attrs.deleteOnSelect == "true")	value = ""
              });
            }
